...
struct SkiaRenderer::Impl {
    ...
    // Per-frame surfaces for each swapchain image
...
void SkiaRenderer::render() {
    ...
    // Get or create surface for this swapchain image
    sk_sp<SkSurface>& surface = m_impl->swapchainSurfaces[imageIndex];
    if (!surface) {
        std::cout << "  Creating surface for swapchain image " << imageIndex << "..." << std::endl;
        ...
        // Create empty VulkanAlloc (swapchain images are not owned)
        ...
        std::cout << "  BackendTexture created (format=" << swapchainFormat << ")" << std::endl;

        // Wrap using the simplified API (colorType inferred from texture format)
        ...
        if (!surface) {
            ...
            std::cout << "  Note: This may fail due to Graphite validation. Falling back to offscreen render." << std::endl;
        } else {
            std::cout << "  Surface created successfully" << std::endl;
        }
    }

    // If we have a valid surface, render to it
    if (surface) {
    ...
    skgpu::graphite::BackendSemaphore signalSemaphore =
        skgpu::graphite::BackendSemaphores::MakeVulkan(renderFinishedSemaphore);
